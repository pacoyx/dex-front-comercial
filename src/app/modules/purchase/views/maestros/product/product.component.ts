...
import { IGetCategoriaProductoShortResponse, IGetUniMedidaShortResponse, IProductListarResponse, ISupplierListarResponse } from '../../../interfaces/IMaestros';
...
export class ProductComponent implements OnInit, AfterViewInit {
  ...
  unidadesMedidas: IGetUniMedidaShortResponse[] = [];
  categoriasProducto: IGetCategoriaProductoShortResponse[] = [];
  ...
  cargarUnidadesMedidas() {
    this.purchaseSettingService.listarUnidadesMedidasCombo().subscribe({
      next: (resp) => {
  ...
  cargarCategoriasProducto() {
    this.purchaseSettingService.listarCategoriasProductoCombo().subscribe({
      next: (resp) => {
        this.categoriasProducto = resp.data;
      },
      error: (err) => {
        console.log(err);
      }
  ...
  editProduct(product: ISupplierListarResponse) {
    const dialogRef = this.dialog.open(DialogformRegProductComponent, {      
      data: { esNuevo: false, objProduct: product, objUM: this.unidadesMedidas, objCP: this.categoriasProducto }
    });
  ...
  nuevoProduct() {
    const dialogRef = this.dialog.open(DialogformRegProductComponent, {      
      data: { esNuevo: true, objCliente: null, objUM: this.unidadesMedidas, objCP: this.categoriasProducto }
    });
